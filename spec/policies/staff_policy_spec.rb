...
describe StaffPolicy do
  ...
  context 'crear un usuario' do
    ...
    context 'crear usuario siendo un ejecutivo' do
      let(:staff) { FactoryGirl.build(:executive) }
      let(:staff2) { nil }
      it { is_expected.to forbid_action(:new) }
      it { is_expected.to forbid_action(:create) }
    end
    ...
    context 'crear usuario siendo un supervisor' do
      let(:staff) { FactoryGirl.build(:supervisor) }
      let(:staff2) { nil }
      it { is_expected.to forbid_action(:new) }
      it { is_expected.to forbid_action(:create) }
    end
    ...
    context 'crear usuario siendo un administrador' do
      let(:staff) { FactoryGirl.build(:admin) }
      let(:staff2) { FactoryGirl.build(:executive) }
    ...
    context 'crear usuario siendo un jefe de sucursal' do
      ...
      staff = FactoryGirl.create(:manager)
      ...
      context 'el usuario a crear es ejecutivo' do
        let(:staff2) { FactoryGirl.build(:executive, :branch_office_id => branch_office) }
        it { is_expected.to permit_action(:create) }
      end

      context 'el usuario a crear es administrador' do
        let(:staff2) { FactoryGirl.build(:admin) }
        it { is_expected.to forbid_action(:create) }
      end

      context 'el usuario a crear es jefe de sucursal' do
        let(:staff2) { FactoryGirl.build(:manager, :branch_office_id => branch_office) }
        it { is_expected.to forbid_action(:create) }
      end

      context 'el usuario a crear es supervisor' do
        let(:staff2) { FactoryGirl.build(:supervisor, :branch_office_id => branch_office) }
        it { is_expected.to permit_action(:create) }
      end

      context 'el usuario a crear es de sucursal distinta que el jefe de sucursal que lo esta creando' do
        let(:staff2) { FactoryGirl.build(:executive, :branch_office_id => branch_office + 1) }
        it { is_expected.to forbid_action(:create) }
      end
  ...
  context 'modificando bloques horarios de un ejecutivo' do
    ...
    context 'siendo un visitor (sin login)' do
      let(:staff) { nil }
      let(:staff2) { FactoryGirl.create(:executive) }
    ...
    context 'siendo supervisor' do
      ...
      supervisor = FactoryGirl.create(:supervisor, :branch_office => branch_office, :attention_type => attention)
      let(:staff) { supervisor }
      let(:staff2) { FactoryGirl.create(:executive, :branch_office => branch_office, :attention_type => attention) }
      it { is_expected.to permit_action(:update_time_blocks) }
    end
